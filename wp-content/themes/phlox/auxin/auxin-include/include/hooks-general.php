 ...
 * @return array
 */
function auxin_modern_comment_form( $comment_form ){
    ...
    $comment_form['fields']        = array(
                'author' => '<div class="aux-input-group"><input type="text"  name="author" id="author" value="'. esc_attr( $commenter['comment_author'] ). '" size="30" '. ( $req ? "aria-required='true' required" : "" ) .' /><label for="author">' . esc_html__( 'Name (required)', 'phlox' ) . '</label><div class="aux-modern-form-border"></div></div>',
                'email'  => '<div class="aux-input-group"><input type="email" name="email"  id="email" value="'. esc_attr( $commenter['comment_author_email'] ). '" size="30" ' . ( $req ? "aria-required='true' required" : "" ) .' /><label for="email">' . esc_html__( 'E-Mail (required)', 'phlox' ) . '</label><div class="aux-modern-form-border"></div></div>',
                'url'    => '<div class="aux-input-group"><input type="url" name="url" id="url" value="'. esc_url( $commenter['comment_author_url'] ). '" size="30" /><label for="url">' . esc_html__( 'Website', 'phlox' ) . '</label><div class="aux-modern-form-border"></div></div>'
            );

    $comment_form['title_reply'] = '<span>' . esc_html__('Add a Comment', 'phlox' ) . '</span>';
    $comment_form['comment_notes_before'] = '<p class="comment-notes">' . __( 'Your email address will not be published. Required fields are marked' ) .'<span class="aux-required-field">*</span></p>';

    return $comment_form;

}

add_filter( 'auxin_default_comment_form', 'auxin_modern_comment_form', 10, 1 );
