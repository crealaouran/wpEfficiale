...
function auxin_the_post_thumbnail( $post_id = null, $width = null , $height = null, $crop = null , $quality = 100, $upscale = false ) {
    ...
    function auxin_get_the_responsive_attachment( $attachment_id = null, $args = array() ) {
        ...
        $defaults = array(
            'quality'         => 100,
            'attr'            => '',
            'preloadable'     => true, // enable this option in order to calculate the image dimensions in page and choose the best image size among image srcset
            ...
            'add_ratio'       => true,
            'image_sizes'     => 'auto',
            'srcset_sizes'    => 'auto', // automatically calculate the image sizes based on the 'size' param, OR 'wp' generates image srcs based on WP default image sizes
            'original_src'    => true
        );
        ...
        } elseif( 'wp' == $srcset_sizes || ( ( is_string( $size ) || empty( $image_aspect_ratio ) ) && 'auto' === $srcset_sizes ) ){
            $default_image_sizes = auxin_base_image_sizes();

            foreach ( $default_image_sizes as $image_size ) {
                $current_image_dimensions = auxin_wp_get_image_size( $image_size );
                $attr_srcset .=  auxin_get_the_resized_attachment_src( $attachment_id, $current_image_dimensions['width'] , $current_image_dimensions['height'], $crop, $quality, $upscale );
                $attr_srcset .= ' '. round( $current_image_dimensions['width'] ).'w,';
            }

        // automatically generate general image sizes based on the aspect ratio of the main image according the dimensions in $size
        } elseif( is_array( $size ) && 'auto' === $srcset_sizes && $image_aspect_ratio ){
            $default_image_sizes = auxin_base_image_sizes();

            foreach ( $default_image_sizes as $image_size ) {
                $general_image_width = get_option( $image_size. '_size_w' );

                $attr_srcset .= auxin_get_the_resized_attachment_src( $attachment_id, $general_image_width, $general_image_width/$image_aspect_ratio, $crop, $quality, $upscale );
                $attr_srcset .= ' '. round( $general_image_width ).'w,';
            }
        }
        ...
        if( $original_src && ( $original_size_img[1] > get_option( 'large_size_w' ) ) ){
        ...
        $html = '';
        $string_size   = $dimensions['width'] . 'x' . $dimensions['height'];
        $hwstring      = $add_hw ? image_hwstring( $dimensions['width'], $dimensions['height'] ) : '';
        ...
        $preload_allowed = $preloadable || auxin_get_option('enable_site_image_preload_preview');

        if( $preload_allowed && 'auto' === $image_sizes ){
        ...
        } elseif( ! $preload_allowed && 'auto' === $image_sizes ) {
            $attr['sizes']  = auxin_generate_image_sizes( array(
                    array( 'min' => '', 'max' => '479px' , 'width' => '480px'  ),
                    array( 'min' => '', 'max' => '767px' , 'width' => '768px'  ),
                    array( 'min' => '', 'max' => '1023px', 'width' => '1024px' ),
                    array( 'min' => '', 'max' => ''      , 'width' => '80vw'  )
                ) 
            );
        }
        ...
        if ( $preload_allowed ) {
            ...
            if( $preload_preview ){
                $attr['src'] = auxin_get_the_resized_attachment_src( $attachment_id, $dimensions['width' ] - 1 , $dimensions['height' ] - 1, $crop, 7, false );
                $attr['data-placeholder'] = basename( $attr['src'] ); // the name of preview image file
...
function auxin_footer_copyright_markup( $echo = false ){
    ...
    if ( auxin_is_true( $attribution ) ) {
        $output .= sprintf( '<small class="attribution"> %1$s <a href="https://wordpress.org/themes/phlox/" title="%2$s"> %3$s </a></small>',
            __( 'Powered by', 'phlox' ),
