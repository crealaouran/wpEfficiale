/*! Auxin WordPress Framework - v2.1.3 (2018-05-15)
...
/* ------------------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------ */
    $.fn.AuxshpCartCanvas = function(){

        var $this = $(this);

        $this.find('.aux-card-dropdown').clone().appendTo('#offcart .aux-offcart-content');
        
        var $burgerBasket = $this.find('#shopping-basket-burger'),
        $burgerBasketIcon = $burgerBasket.find('>.aux-cart-contents'),  
        $basketContainer  = $this.find('#offcart'),
        isClosed          = true;
        ...
        function toggleOffcanvasBasket() {
            ...
            $this.toggleClass( 'aux-offcanvas-overlay' );
            isClosed = !isClosed;
        }
        ...
        $window.on( 'resize', function() {
            if ( window.innerWidth > 767 ) {
                ...
                if( !isClosed ){
                    $this.removeClass( 'aux-offcanvas-overlay' );
                }                
            } else {
                $basketContainer.show();
                if( !isClosed ){
                    $this.addClass( 'aux-offcanvas-overlay' );
                }                
            }
        });

    }
    $(document.body).AuxshpCartCanvas();
    ...
    $.fn.auxModernForm = function(){
        var groupClass = '.aux-input-group',
            focusClass = 'aux-focused',
            $allFields = $(this).find( groupClass + ' input ,' + groupClass + ' textarea' );
    ...
    /* ------------------------------------------------------------ */
    // Scale Divs By Scroll
    /* ------------------------------------------------------------ */

    $.fn.auxScaleDivs = function( start, target, startScale, endScale ){
        var $this        = $(this),
            $el          = $(start),
            $target      = $(target),
            endScale     = endScale || 1 ,
    ...
    $('.aux-bs-get-started').each(function() {
        $(this).auxScaleDivs( '.aux-bs-footer-scale', '.aux-subfooter .aux-wrapper', 0.94, 1 );
    });
    ...
    $.fn.scrollBlurImage = function( blurValue, startFrom, opacitySpeed ){
        var $this =  $(this),
    ...
    $('.aux-blur-fade').each(function() {
        $(this).scrollBlurImage( 15, 0.3, 4 );
    });
    ...
    $.fn.pageCoverAnimation = function(){
        var $this      = $(this),
            fired      = false,
            scrollLock = true;
    ...
    $('.aux-page-cover-wrapper').each(function() {
        $(this).pageCoverAnimation();
    });
